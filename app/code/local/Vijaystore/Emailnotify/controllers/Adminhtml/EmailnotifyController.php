...
class Vijaystore_Emailnotify_Adminhtml_EmailnotifyController extends Mage_Adminhtml_Controller_Action
{
    ...
    {
        $this->loadLayout()->_setActiveMenu('emailnotify/items')->_addBreadcrumb(Mage::helper('adminhtml')->__('Contact Form Fields '), Mage::helper('adminhtml')->__('Email Notify'));
    ...
    public function massResendAction()
    {
        ...
        Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('adminhtml')->__('Total of %d record(s) were successfully Sent Mail', count($notify_succ)));
        $this->_redirect('*/*/index');
    }
    ...
    public function viewAction()
    {
        ...
        if ($emailnotifyModel->getId() || $emailnotifyId == 0) {
            ...
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('emailnotify')->__('Record does not exist'));
            $this->_redirect('*/*/');
        }
    }
    ...
    public function massDeleteAction()
    {
        ...
        if (!is_array($emailnotifys)) {
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('adminhtml')->__('Total of %d record(s) were successfully deleted', count($emailnotifys)));
    ...
    {
        $this->loadLayout();
        $this->getResponse()->setBody($this->getLayout()->createBlock('emailnotify/adminhtml_emailnotify_grid')->toHtml());
    }
